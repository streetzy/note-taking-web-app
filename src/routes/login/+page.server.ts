...
import { create_session, generate_session_token, set_session_token_cookie } from '$lib/server/session.js';
import { get_user_from_email, get_user_password_hash } from '$lib/server/user.js';
...
async function action(event: RequestEvent) {
    ...
    const valid_pw = await verify_password_hash(password_hash.toString(), password); // again, we do not mention me using String instead of string and then just being lazy
    if (!valid_pw) {
        return fail(400, {

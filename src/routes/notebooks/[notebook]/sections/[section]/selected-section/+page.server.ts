...
export const load : PageServerLoad = async ({ params, locals }) => {
    ...
    const section_pages = await notebook_section_pages(params.notebook, params.section);

    return { notebook_id: params.notebook, section_id: params.section, notebook_name: _notebook_name, section_name: _section_name, pages: section_pages };
}
...
export const actions = {
    default: async ({params, locals, request}) => {
        ...
        return redirect(302, `/notebooks/${params.notebook}/sections/${params.section}/selected-section`);
    }
}
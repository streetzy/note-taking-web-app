...
import { delete_password_reset_session_token_cookie, invalidate_user_password_reset_sessions, validate_password_reset_session_request } from "$lib/server/password-reset";
import { create_session, generate_session_token, invalidate_user_sessions, set_session_token_cookie } from "$lib/server/session.js";
...
async function action(event: RequestEvent) {
    ...
    const _session = await create_session(session_token, session.user_id);
